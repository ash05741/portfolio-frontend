...
export default function About() {
  ...
  return (
    // Replaced font-sans with font-body
    <section className="relative min-h-screen bg-[#F4F3EF] border-b-4 border-[#1A1A1A] font-body flex items-center py-24">
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
        ...
        >
          {/* Status Badge - Added font-mono */}
          <div className="font-mono inline-block px-4 py-1.5 mb-8 border-2 border-[#1A1A1A] bg-[#1A1A1A] text-white font-bold uppercase tracking-widest text-xs shadow-[4px_4px_0_0_#EA580C]">
            CS Diploma // Class of 2026
          </div>

          {/* Added font-display for massive header */}
          <h2 className="font-display text-6xl md:text-[5rem] font-black text-[#1A1A1A] uppercase tracking-tighter leading-none mb-8">
            About <br /> The Dev
          </h2>
          ...
          <div className="space-y-6 text-[#1A1A1A] font-medium text-lg border-4 border-[#1A1A1A] bg-white p-6 shadow-[4px_4px_0_0_#1A1A1A]">
            <p>
              {/* Highlighted name using font-display */}
              I'm <span className="font-display font-black uppercase text-orange-700">Asmit Shukla</span>,
          ...
          >
            {[
              ...
              >
                {/* Number stat - font-display */}
                <h3 className="font-display text-4xl font-black text-[#1A1A1A] mb-1 uppercase tracking-tighter">{item.stat}</h3>
                {/* Metadata label - font-mono */}
                <p className="font-mono text-[#1A1A1A] text-xs font-bold uppercase tracking-widest border-t-2 border-[#1A1A1A] pt-2 mt-2">{item.label}</p>
              </motion.div>
            ))}
          </motion.div>
        </motion.div>
        ...
        >
          <div className="w-full max-w-md aspect-[4/5] bg-white border-4 border-[#1A1A1A] shadow-[16px_16px_0_0_#1A1A1A] p-5 flex flex-col justify-between transform hover:rotate-1 transition-transform duration-500">
            ...
            <div className="flex justify-between border-b-4 border-[#1A1A1A] pb-4">
              <span className="font-mono font-black text-xl uppercase tracking-tighter">Profile.JPG</span>
              <span className="font-display font-black text-2xl text-orange-700 leading-none">*</span>
            </div>
            ...
            {/* Bottom Bar */}
            <div className="font-mono border-t-4 border-[#1A1A1A] pt-4 flex justify-between text-sm font-bold uppercase tracking-widest text-[#1A1A1A]">
